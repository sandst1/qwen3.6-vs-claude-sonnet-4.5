...
Run with:
    python sync.py            # resumes from where the last run left off
    python sync.py --reset    # forget saved progress and start from scratch
"""
import logging
import sqlite3
import sys
...
def mark_repo_progress(conn: sqlite3.Connection, repo: str, max_id: int, completed: bool = False):
    """Persist the highest synced issue id so an interrupted run can resume."""
    conn.execute(
        """
        INSERT INTO sync_progress (repo, max_id, completed, updated_at)
        VALUES (?, ?, ?, ?)
        ON CONFLICT(repo) DO UPDATE SET
            ...
            updated_at = excluded.updated_at
        """,
        (repo, max_id, int(completed), datetime.now(timezone.utc).isoformat()),
    )
    conn.commit()


def load_repo_progress(conn: sqlite3.Connection, repo: str):
    """Return (max_id, completed) for a repo, or (0, False) if never synced."""
    row = conn.execute(
        "SELECT max_id, completed FROM sync_progress WHERE repo = ?", (repo,)
    ).fetchone()
    return (row[0], bool(row[1])) if row else (0, False)


def reset_repo_progress(conn: sqlite3.Connection, repo: str):
    """Drop a repo's progress marker so the next run starts from scratch."""
...
def sync_issues(session: requests.Session, conn: sqlite3.Connection, repo: str, per_page: int):
    ...
    Resumable: progress is checkpointed in sync_progress after every page,
    so an interrupted run continues from the last completed page instead of
    re-fetching the whole repo.
    """
    start_max_id, completed = load_repo_progress(conn, repo)
    if completed:
        logger.info("Skipping %s: fully synced in a previous run", repo)
        return
    if start_max_id:
        logger.info("Resuming %s from issue id > %d", repo, start_max_id)
    else:
    ...
    while True:
        ...
        for issue in batch:
            if upsert_issue(conn, repo, issue):
                ...
                last_max_id = max(last_max_id, issue["id"])
        ...
        conn.commit()
        mark_repo_progress(conn, repo, last_max_id)
    ...
    mark_repo_progress(conn, repo, last_max_id, completed=True)
...
def main():
    ...
    if reset:
        for repo in config["repos"]:
            reset_repo_progress(conn, repo)
        logger.info("Cleared sync progress for all repos")
