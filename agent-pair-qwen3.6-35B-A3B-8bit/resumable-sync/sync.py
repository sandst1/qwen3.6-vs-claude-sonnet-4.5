...
def get_sync_state(conn: sqlite3.Connection, repo: str) -> dict:
    """Return the last known sync state for a repo, or defaults."""
    row = conn.execute(
        "SELECT status, last_page, total_pages FROM sync_state WHERE repo = ?",
        (repo,),
    ).fetchone()
    if row is None:
        return {"status": "new", "last_page": 0, "total_pages": None}
    return {
        "status": row[0],
        "last_page": row[1],
        "total_pages": row[2],
...
def update_sync_state(
    ...
    conn.execute(
        """
        INSERT INTO sync_state (repo, status, last_page, total_pages, updated_at)
        VALUES (?, ?, ?, ?, ?)
        ...
        """,
        (repo, status, last_page, total_pages, now),
    )
    conn.commit()
...
def sync_comment_pages(session: requests.Session, conn: sqlite3.Connection, issue: dict):
    """Fetch all comment pages for a single issue."""
...
def sync_issues(session: requests.Session, conn: sqlite3.Connection, repo: str, per_page: int):
    ...
    if state["status"] == "completed":
        logger.info("Skipping already synced repo: %s", repo)
        return

    if state["last_page"] > 1:
        logger.info("Resuming %s from page %d", repo, state["last_page"])
    elif state["status"] == "syncing":
        logger.info("Resuming %s", repo)

    start_page = max(state["last_page"], 1)
    ...
    while True:
        ...
        for issue in batch:
            if upsert_issue(conn, repo, issue):
                total_issues_this_page += 1

                # If the issue has comments, fetch them too
                if issue.get("comments", 0) > 0:
                    sync_comment_pages(session, conn, issue)

        conn.commit()

        total_issues += total_issues_this_page

        # --- Persist state BEFORE fetching the next page ------------------
        update_sync_state(conn, repo, page, None, status="syncing")
        # ----------------------------------------------------------------

        logger.info(
            "Page %d: %d issues (total so far: %d)",
            page, total_issues_this_page, total_issues,
        )

        if len(batch) < per_page:
            # Last page — mark as completed
            update_sync_state(conn, repo, page, None, status="completed")
            break
...
def main():
    ...
    try:
        ...
        # State is persisted at the end of each page loop in sync_issues,
        # so the next run will resume from the last completed page.
        logger.warning("Interrupted. Resume on next run.")
        sys.exit(130)
    except Exception:
