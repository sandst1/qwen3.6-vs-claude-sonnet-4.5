...
def plan_run(conn: sqlite3.Connection, repos: list):
    """Decide whether to resume an interrupted run or start a fresh one.

    If any repo is still marked in_progress, a previous run was interrupted
    (restart, Ctrl-C, exhausted retries) — resume it from its last committed
    page and skip repos it already finished. Otherwise start a new run.

    Returns (run_id, {repo: start_page}); repos finished in the interrupted
    run are omitted from the mapping.
    """
    if not repos:
        return uuid.uuid4().hex, {}

    placeholders = ",".join("?" * len(repos))
    conn.execute(f"DELETE FROM sync_state WHERE repo NOT IN ({placeholders})", repos)

    rows = conn.execute(
        "SELECT repo, run_id, status, next_page FROM sync_state"
    ).fetchall()
    state = {row[0]: row for row in rows}
    interrupted = [row for row in state.values() if row[2] == "in_progress"]

    if interrupted:
        run_id = interrupted[0][1]
        start_pages = {}
        for repo in repos:
            if repo not in state:
                start_pages[repo] = 1
            elif state[repo][2] == "in_progress":
                start_pages[repo] = state[repo][3]
            # completed in the interrupted run -> nothing left to sync
        logger.info(
            "Resuming interrupted run %s: %s",
            run_id,
            {r: p for r, p in start_pages.items()} or "nothing left",
        )
    else:
        run_id = uuid.uuid4().hex
        start_pages = {}
        for repo in repos:
            checkpoint(conn, repo, run_id, 1)
            start_pages[repo] = 1
        logger.info("Starting new run %s for %d repo(s)", run_id, len(repos))

    conn.commit()
    return run_id, start_pages
...
def sync_issues(session: requests.Session, conn: sqlite3.Connection, repo: str, per_page: int, run_id: str, start_page: int = 1):
    """Fetch all issues for a repo and write them to the DB.

    Progress is checkpointed after every page in the same transaction as the
    data, so an interrupted run can resume from the next page. Upserts are
    idempotent, so re-syncing a page after a crash is harmless.
    """
    if start_page > 1:
        logger.info("Syncing issues for %s (resuming at page %d)", repo, start_page)
    else:
    ...
    checkpoint(conn, repo, run_id, 1, status="completed")
    conn.commit()
...
def main():
    ...
    try:
        run_id, start_pages = plan_run(conn, config["repos"])
        for repo in config["repos"]:
            if repo not in start_pages:
                logger.info("Skipping %s (already synced in run %s)", repo, run_id)
                continue
            sync_issues(
                session,
                conn,
