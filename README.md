# Benchmark: Twenty-Two-Model Comparison (Qwen + Sonnet + DeepSeek + Step + Ornith + A1 + Ternary Bonsai + Laguna + Muse Glimmer)

A side-by-side comparison of twenty-two AI model variants on non-trivial coding tasks designed to test **architectural judgment** and **ambiguity-handling** - not just raw correctness.
...
Fourth: **[Qwen 3.8-27B-FP8 (reasoning-medium)](https://huggingface.co/Qwen/Qwen3.8-27B-FP8)** at 68/80 — same FP8 checkpoint with thinking mode on and `reasoning_effort="medium"`. Sync (34/40) loses 2 points to scope creep (incremental sync via `cutoff` timestamp conflated with crash recovery); widgets (34/40) gain 1 point over thinking-off via a good README and native drag-and-drop.  
Fifth: **Qwen 3.6-27B-8bit (Local, unsloth)** at 66.5/80 — the previous leader, now displaced by the three co-winners.  
Sixth: **Claude Sonnet 4.5** at 65/80.  
Seventh: **[Laguna S 2.1-NVFP4](https://huggingface.co/poolside/Laguna-S-2.1-NVFP4)** at 64.5/80 — Poolside's 117.6B MoE (8.5B activated) in NVFP4, run with recommended sampling (`temperature 0.7`, `top_p 0.95`) and thinking disabled.  
Eighth: **Qwen 3.6-35B-A3B-8bit (Local, agent-pair)** at 64/80 — the same 35B-A3B architecture in FP8 quantization, run with agent-pair observer feedback.  
Ninth: **[Qwen 3.6-27B-NVFP4](https://huggingface.co/nvidia/Qwen3.6-27B-NVFP4)** at 63/80 — NVIDIA's official FP4 quantization of Qwen 3.6-27B, run locally via vLLM.  
Tenth: **[DeepSeek-V4-Flash-REAP-180B](https://huggingface.co/0xSero/DeepSeek-V4-Flash-180B)** at 62/80 — a REAP-pruned 180B MoE running on a single DGX Spark.  
Eleventh: **Qwen 3.6-27B-NVFP4 (unsloth)** at 61/80 — the same NVFP4 quantization of Qwen 3.6-27B run locally via unsloth rather than vLLM, scoring 2 points below the NVIDIA official build.  
Twelfth: **[Qwen 3.8-27B-4bit (unsloth)](https://huggingface.co/unsloth/Qwen3.8-27B-GGUF)** at 60/80 — Unsloth Dynamic V3.0 `UD-Q4_K_XL` GGUF (~17.9 GB) of Qwen 3.8-27B, run locally via llama.cpp with thinking-mode disabled. Widgets (34/40) tie Q6K for 4th-best; sync (26/40) drops 10 points from the FP8 co-winner due to a critical `since` bug (passes issue ID to GitHub's timestamp parameter).  
...
**[Muse Glimmer 30B K-Quant Dynamic](https://huggingface.co/meta-models/Muse-Glimmer-30B-GGUF/)** at 56/80 — Meta's 29.6B dense agentic model, run locally from `muse-glimmer-30B-kquant-dynamic.gguf`. Strong widget structure (31/40) but a lightly reasoned, undocumented sync implementation (25/40) place it 17th.  
...
**[Ternary Bonsai 27B](https://huggingface.co/prism-ml/Ternary-Bonsai-27B-gguf)** at 50.5/80 — a 1.71 bits/weight ternary model (~7.2 GB, llama.cpp Q2_0); strong widget architecture but `since`-based sync conflation and a blocking import bug place it 20th.  
**Agents A1 FP8** at 48/80 — per-issue checkpointing (not per-page) on the sync task and type/instance conflation on widgets place it 21st.  
...
├── qwen3.8-27B-8bit-reasoning-medium/   # Qwen 3.8-27B-FP8 — #4 (68/80); reasoning_effort=medium
│   ├── resumable-sync/
│   ├── pluggable-widgets/
│   ├── review-resumable-sync.md
│   └── review-pluggable-widgets.md
├── qwen3.8-27B-4bit/                    # Qwen 3.8-27B UD-Q4_K_XL (unsloth) — 60/80 (12th); thinking off
│   ├── resumable-sync/
│   ├── pluggable-widgets/
│   ├── review-resumable-sync.md
│   └── review-pluggable-widgets.md
...
**[Qwen 3.8-27B-FP8](https://huggingface.co/Qwen/Qwen3.8-27B-FP8), Qwen 3.6-27B-Q6K (unsloth), and [DeepSeek-V4-Flash-0731 IQ3_XXS](https://huggingface.co/unsloth/DeepSeek-V4-Flash-0731-GGUF) tie for first (69/80).** [Qwen 3.8-27B-FP8 (reasoning-medium)](https://huggingface.co/Qwen/Qwen3.8-27B-FP8) places 4th at 68/80 — same FP8 checkpoint with `reasoning_effort="medium"`: sync loses 2 points to incremental-sync scope creep, widgets gain 1 point via README and drag-and-drop. [Qwen 3.8-27B-4bit (unsloth)](https://huggingface.co/unsloth/Qwen3.8-27B-GGUF) places 12th at 60/80 — widgets hold up (34/40, tying Q6K) but sync drops 10 points from FP8 due to a `since` API bug. [Muse Glimmer 30B Q4_K_XL (unsloth)](https://huggingface.co/unsloth/Muse-Glimmer-30B-GGUF) places 16th at 57/80, narrowly ahead of the official K-Quant Dynamic build (56/80, 17th).
...
| 4 | [Qwen 3.8-27B-FP8 (reasoning-medium)](https://huggingface.co/Qwen/Qwen3.8-27B-FP8) | 68/80 |
| 5 | Qwen 3.6-27B-8bit (Local) | 66.5/80 |
| 6 | Claude Sonnet 4.5 | 65/80 |
| 7 | [Laguna S 2.1-NVFP4](https://huggingface.co/poolside/Laguna-S-2.1-NVFP4) | 64.5/80 |
| 8 | Qwen 3.6-35B-A3B-8bit (Local, agent-pair) | 64/80 |
| 9 | [Qwen 3.6-27B-NVFP4](https://huggingface.co/nvidia/Qwen3.6-27B-NVFP4) | 63/80 |
| 10 | [DeepSeek-V4-Flash-REAP-180B](https://huggingface.co/0xSero/DeepSeek-V4-Flash-180B) | 62/80 |
| 11 | Qwen 3.6-27B-NVFP4 (unsloth) | 61/80 |
| 12 | [Qwen 3.8-27B-4bit (unsloth)](https://huggingface.co/unsloth/Qwen3.8-27B-GGUF) | 60/80 |
| 13 (tie) | Qwen 3.6-27B-4bit | 59/80 |
| 13 (tie) | Qwen 3.6-35B-A3B-4bit | 59/80 |
| 15 | Qwen 3.6-27B-AEON-NVFP4 | 57.5/80 |
| 16 | [Muse Glimmer 30B Q4_K_XL (unsloth)](https://huggingface.co/unsloth/Muse-Glimmer-30B-GGUF) | 57/80 |
| 17 | [Muse Glimmer 30B K-Quant Dynamic](https://huggingface.co/meta-models/Muse-Glimmer-30B-GGUF/) | 56/80 |
| 18 | Qwen 3.6-27B-4bit (OpenRouter) | 54/80 |
| 19 | [Ornith-1.0-35B-8bit](https://huggingface.co/deepreinforce-ai/Ornith-1.0-35B-GGUF) | 51/80 |
| 20 | [Ternary Bonsai 27B](https://huggingface.co/prism-ml/Ternary-Bonsai-27B-gguf) | 50.5/80 |
| 21 | Agents A1 FP8 | 48/80 |
| 22 | [Step-3.7-Flash IQ4_XS](https://huggingface.co/stepfun-ai/Step-3.7-Flash-GGUF) | 46/80 |
...
- **Qwen 3.8-27B-FP8 (reasoning-medium)** at 68/80 shows a thinking-on tradeoff on the same checkpoint: sync drops 2 points (incremental-sync scope creep) while widgets gain 1 point (README + drag-and-drop) vs thinking-off.
...
- **Ambiguity-resolution remains the hardest bar** across all twenty-two models — most implementations pick a path and code it without clearly naming tradeoffs.
