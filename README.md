# Benchmark: Twenty-Five-Model Comparison (Qwen + Opus + Sonnet + DeepSeek + Step + Ornith + A1 + Ternary Bonsai + Laguna + Muse Glimmer)

A side-by-side comparison of twenty-four AI model variants on non-trivial coding tasks designed to test **architectural judgment** and **ambiguity-handling** - not just raw correctness.
...
| **[Qwen3.8-Flash-Next UD-Q3_K_XL (unsloth)](https://huggingface.co/unsloth/Qwen3.8-Flash-Next-GGUF)** | 31 / 40 | 38 / 40 | **69** | 86.25% |
...
| **Claude Opus 4.6 (GitHub Copilot)** | 33 / 40 | 33.5 / 40 | **66.5** | 83.1% |
...
**Four runners-up at 69/80:** [Qwen3.8-Flash-Next UD-Q3_K_XL (unsloth)](https://huggingface.co/unsloth/Qwen3.8-Flash-Next-GGUF), [Qwen 3.8-27B-FP8](https://huggingface.co/Qwen/Qwen3.8-27B-FP8), Qwen 3.6-27B-Q6K (unsloth), and [DeepSeek-V4-Flash-0731 IQ3_XXS](https://huggingface.co/unsloth/DeepSeek-V4-Flash-0731-GGUF) — complementary strengths. Flash-Next’s 38/40 widgets score is the second-highest in the benchmark, with a complete plugin contract, type/instance separation, and user customization; its focused per-page sync scores 31/40. Qwen 3.8-27B-FP8 still has the highest sync score (36/40) with atomic page+checkpoint transactions and full code quality; DS-V4-Flash-0731 has the previous second-highest widget score (37/40); Q6K pairs strong widgets (34/40) with a unique `max_commented_issue_id` high-water mark on sync (35/40). Qwen 3.8-27B-FP8 was run locally from the official FP8 checkpoint with thinking-mode disabled.
...
Sixth (tie): **Qwen 3.6-27B-8bit (Local, unsloth)** and **Claude Opus 4.6 (GitHub Copilot)** at 66.5/80 — complementary splits. Local 8-bit is weaker on sync (31/40) and stronger on widgets (35.5/40); Opus matches Sonnet on sync (33/40) with atomic page+checkpoint transactions and no incremental-sync creep, and matches Laguna on widgets (33.5/40).  
Eighth: **Claude Sonnet 4.5** at 65/80 — 1.5 points behind Opus; same sync score (33/40) but 1.5 points weaker on widgets (32 vs 33.5).  
Ninth: **[Laguna S 2.1-NVFP4](https://huggingface.co/poolside/Laguna-S-2.1-NVFP4)** at 64.5/80 — Poolside's 117.6B MoE (8.5B activated) in NVFP4, run with recommended sampling (`temperature 0.7`, `top_p 0.95`) and thinking disabled.  
Tenth: **Qwen 3.6-35B-A3B-8bit (Local, agent-pair)** at 64/80 — the same 35B-A3B architecture in FP8 quantization, run with agent-pair observer feedback.  
Eleventh: **[Qwen 3.6-27B-NVFP4](https://huggingface.co/nvidia/Qwen3.6-27B-NVFP4)** at 63/80 — NVIDIA's official FP4 quantization of Qwen 3.6-27B, run locally via vLLM.  
Twelfth: **[DeepSeek-V4-Flash-REAP-180B](https://huggingface.co/0xSero/DeepSeek-V4-Flash-180B)** at 62/80 — a REAP-pruned 180B MoE running on a single DGX Spark.  
Thirteenth: **Qwen 3.6-27B-NVFP4 (unsloth)** at 61/80 — the same NVFP4 quantization of Qwen 3.6-27B run locally via unsloth rather than vLLM, scoring 2 points below the NVIDIA official build.  
Fourteenth: **[Qwen 3.8-27B-4bit (unsloth), thinking off](https://huggingface.co/unsloth/Qwen3.8-27B-GGUF)** at 60/80 — same `UD-Q4_K_XL` GGUF as the winner, but with thinking-mode disabled. Widgets (34/40) still tie Q6K; sync (26/40) drops 8 points from the reasoning-medium run due to a critical `since` bug (passes issue ID to GitHub's timestamp parameter).  
...
**[Muse Glimmer 30B K-Quant Dynamic](https://huggingface.co/meta-models/Muse-Glimmer-30B-GGUF/)** at 56/80 — Meta's 29.6B dense agentic model, run locally from `muse-glimmer-30B-kquant-dynamic.gguf`. Strong widget structure (31/40) but a lightly reasoned, undocumented sync implementation (25/40) place it 19th.  
...
**[Ternary Bonsai 27B](https://huggingface.co/prism-ml/Ternary-Bonsai-27B-gguf)** at 50.5/80 — a 1.71 bits/weight ternary model (~7.2 GB, llama.cpp Q2_0); strong widget architecture but `since`-based sync conflation and a blocking import bug place it 22nd.  
**Agents A1 FP8** at 48/80 — per-issue checkpointing (not per-page) on the sync task and type/instance conflation on widgets place it 23rd.  
...
> **Note on Claude Opus 4.6:** Run via GitHub Copilot. Scores 66.5/80 — ties Qwen 3.6-27B-8bit for 7th of 25, 1.5 points above Sonnet 4.5 (65/80). Sync (33/40) matches Sonnet: per-page `sync_state` with `get_resume_page()`/`save_sync_progress()`/`clear_sync_state()`, page data and cursor committed in the same transaction, `--full` flag, no incremental-sync scope creep. Full marks on existing-code respect (8/8) and strong failure-mode reasoning (5/6). Loses points on silent granularity choice (1/4 for naming the ambiguity), no README update (docstring only), and not arguing why the comments subloop needs no inner checkpoint. Widgets (33.5/40) match Laguna: clean `WidgetDefinition` contract, Map registry with `registerWidget()`, both user customization and developer extensibility implemented, consistent span-via-wrapper refactoring, and full code quality (4/4). Held back by type/instance conflation (layout is `string[]` of type IDs; React keys are `${type}-${idx}`), silent user-vs-developer ambiguity (8/10 — implemented both, named neither), no settings on the contract, an unversioned localStorage key (`dashboard-layout`), and no widget README update (adding-a-widget guide lives in `widgets/index.ts`).

> **Note on Qwen 3.8-27B-4bit (unsloth), reasoning-medium:** Same [unsloth/Qwen3.8-27B-GGUF](https://huggingface.co/unsloth/Qwen3.8-27B-GGUF) `Qwen3.8-27B-UD-Q4_K_XL.gguf` quant (~17.9 GB) as the thinking-off 4bit run, but with thinking mode on and `reasoning_effort="medium"`. Sole winner at 73/80 (1st of 25) — 13 points above thinking-off 4bit (60/80) and 4 points above the previous 69/80 pack. Widgets (39/40) are the benchmark best: perfect `WidgetDefinition`/`WidgetInstance` split with unique `key`s (`crypto.randomUUID()`), Map registry with `registerWidget()` and duplicate-detection throw, both user-vs-developer forks named and implemented in the README, versioned localStorage (`ops-dashboard.layout.v1`) with registry validation, native HTML5 drag-and-drop, consistent span-via-wrapper refactoring, and full marks on existing-code respect (8/8), frontend craft (4/4), and code quality (4/4). The only deduction is no settings abstraction on the widget contract (5/6). Sync (34/40) ties FP8 reasoning-medium: per-page `sync_state` with `get_sync_state()`/`set_sync_state()`, implicit atomic page+cursor commits, `--full` flag, and a comprehensive README. Incremental sync via GitHub's `since` is scope creep, but unlike thinking-off 4bit it uses a real ISO timestamp watermark and documents incremental refresh as separate from crash recovery.

> **Note on Qwen 3.8-27B-4bit (unsloth), thinking off:** Same [unsloth/Qwen3.8-27B-GGUF](https://huggingface.co/unsloth/Qwen3.8-27B-GGUF) `Qwen3.8-27B-UD-Q4_K_XL.gguf` quant, run locally via llama.cpp with thinking-mode disabled. It scores 60/80 (15th of 25) — 13 points below the reasoning-medium run on the same file. The split is stark: widgets (34/40) still tie Q6K with a clean `WidgetDef`/`WidgetPlacement` type split, Map registry with duplicate detection, full frontend craft (4/4), and a good widget README; sync (26/40) has the right per-page `sync_progress` structure and excellent README documentation but is undermined by passing an integer issue ID to GitHub's `since` parameter (which expects an ISO 8601 timestamp) and writing checkpoints in a separate transaction from page data.

> **Note on Qwen3.8-Flash-Next UD-Q3_K_XL (unsloth):** This is [Unsloth's Qwen3.8-Flash-Next GGUF](https://huggingface.co/unsloth/Qwen3.8-Flash-Next-GGUF), specifically the `UD-Q3_K_XL` quantization (~90 GB). It scores 69/80 (tied for 2nd of 25): 31/40 on sync and 38/40 on widgets. Sync is a focused, correct per-page SQLite checkpoint design: state is kept per repository in `sync_state`, page data and the next-page cursor are committed together, and the README documents safe idempotent retries. It loses points by not explaining the per-page tradeoff, by leaving the comment subloop implicit, and by providing no state-schema versioning. Widgets are the second-highest score in the benchmark: a typed `WidgetPlugin` contract, central registry, real `WidgetInstance` IDs that permit duplicate types, versioned and validated localStorage persistence, native drag-and-drop, and a complete catalog/add-remove-resize workflow. The only meaningful widget deduction is that it implements both user customization and developer extensibility without explicitly naming that ambiguity.
...
│   ├── review-resumable-sync.md
│   └── review-pluggable-widgets.md
├── qwen3.8-27B-4bit/                    # Qwen 3.8-27B UD-Q4_K_XL (unsloth) — 60/80 (14th); thinking off
│   ├── resumable-sync/
│   ├── pluggable-widgets/
│   ├── review-resumable-sync.md
│   └── review-pluggable-widgets.md
...
│   ├── review-resumable-sync.md
│   └── review-pluggable-widgets.md
├── qwen3.8-flash-next-ud_q3_xl/         # Qwen3.8-Flash-Next UD-Q3_K_XL (unsloth) — #2 (69/80)
│   ├── resumable-sync/
│   ├── pluggable-widgets/
│   ├── review-resumable-sync.md
│   └── review-pluggable-widgets.md
├── qwen3.6-27B-8bit/                    # Qwen 3.6-27B 8-bit local (unsloth) — 66.5/80 (6th, tie)
│   ├── resumable-sync/
│   ├── pluggable-widgets/
│   ├── review-resumable-sync.md
│   └── review-pluggable-widgets.md
...
│   ├── review-resumable-sync.md
│   └── review-pluggable-widgets.md
├── opus-4.6/                            # Claude Opus 4.6 (GitHub Copilot) — 66.5/80 (6th, tie)
│   ├── resumable-sync/
│   ├── pluggable-widgets/
│   ├── review-resumable-sync.md
│   └── review-pluggable-widgets.md
...
│   ├── review-resumable-sync.md
│   └── review-pluggable-widgets.md
├── ternary-bonsai-27b/                  # Ternary Bonsai 27B Q2_0 (llama.cpp) — 50.5/80 (22nd)
│   ├── resumable-sync/
│   ├── pluggable-widgets/
│   ├── review-resumable-sync.md
│   └── review-pluggable-widgets.md
├── laguna-s2.1-nvfp4/                   # Laguna S 2.1-NVFP4 (Poolside) — 64.5/80 (9th); thinking off
│   ├── resumable-sync/
│   ├── pluggable-widgets/
│   ├── review-resumable-sync.md
│   └── review-pluggable-widgets.md
├── muse-glimmer-30b/                    # Muse Glimmer 30B K-Quant Dynamic — 56/80 (19th)
│   ├── resumable-sync/
│   ├── pluggable-widgets/
│   ├── review-resumable-sync.md
│   └── review-pluggable-widgets.md
└── unsloth-muse-glimmer-30B-Q4_K_XL/    # Muse Glimmer 30B Q4_K_XL (unsloth) — 57/80 (18th)
    ├── resumable-sync/
    ├── pluggable-widgets/
    ├── review-resumable-sync.md
    └── review-pluggable-widgets.md
...
**[Qwen 3.8-27B-4bit (unsloth), reasoning-medium](https://huggingface.co/unsloth/Qwen3.8-27B-GGUF) wins outright at 73/80.** [Qwen3.8-Flash-Next UD-Q3_K_XL (unsloth)](https://huggingface.co/unsloth/Qwen3.8-Flash-Next-GGUF), [Qwen 3.8-27B-FP8](https://huggingface.co/Qwen/Qwen3.8-27B-FP8), Qwen 3.6-27B-Q6K (unsloth), and [DeepSeek-V4-Flash-0731 IQ3_XXS](https://huggingface.co/unsloth/DeepSeek-V4-Flash-0731-GGUF) tie for second (69/80). [Qwen 3.8-27B-FP8 (reasoning-medium)](https://huggingface.co/Qwen/Qwen3.8-27B-FP8) places 6th at 68/80. **Claude Opus 4.6 (GitHub Copilot)** ties Qwen 3.6-27B-8bit for 7th at 66.5/80 — 1.5 points above Sonnet 4.5 (65/80, 9th): same sync score as Sonnet (33/40) and 1.5 points stronger on widgets (33.5 vs 32). [Qwen 3.8-27B-4bit (unsloth), thinking off](https://huggingface.co/unsloth/Qwen3.8-27B-GGUF) places 15th at 60/80 — same GGUF file as the winner, but thinking disabled. [Muse Glimmer 30B Q4_K_XL (unsloth)](https://huggingface.co/unsloth/Muse-Glimmer-30B-GGUF) places 19th at 57/80, narrowly ahead of the official K-Quant Dynamic build (56/80, 20th).

| Rank | Model | Total |
|------|-------|-------|
| 1 | [Qwen 3.8-27B-4bit (unsloth), reasoning-medium](https://huggingface.co/unsloth/Qwen3.8-27B-GGUF) | 73/80 |
| 2 (tie) | [Qwen3.8-Flash-Next UD-Q3_K_XL (unsloth)](https://huggingface.co/unsloth/Qwen3.8-Flash-Next-GGUF) | 69/80 |
...
| 6 | [Qwen 3.8-27B-FP8 (reasoning-medium)](https://huggingface.co/Qwen/Qwen3.8-27B-FP8) | 68/80 |
| 7 (tie) | Qwen 3.6-27B-8bit (Local) | 66.5/80 |
| 7 (tie) | Claude Opus 4.6 (GitHub Copilot) | 66.5/80 |
| 9 | Claude Sonnet 4.5 | 65/80 |
| 10 | [Laguna S 2.1-NVFP4](https://huggingface.co/poolside/Laguna-S-2.1-NVFP4) | 64.5/80 |
| 11 | Qwen 3.6-35B-A3B-8bit (Local, agent-pair) | 64/80 |
| 12 | [Qwen 3.6-27B-NVFP4](https://huggingface.co/nvidia/Qwen3.6-27B-NVFP4) | 63/80 |
| 13 | [DeepSeek-V4-Flash-REAP-180B](https://huggingface.co/0xSero/DeepSeek-V4-Flash-180B) | 62/80 |
| 14 | Qwen 3.6-27B-NVFP4 (unsloth) | 61/80 |
| 15 | [Qwen 3.8-27B-4bit (unsloth), thinking off](https://huggingface.co/unsloth/Qwen3.8-27B-GGUF) | 60/80 |
| 16 (tie) | Qwen 3.6-27B-4bit | 59/80 |
| 16 (tie) | Qwen 3.6-35B-A3B-4bit | 59/80 |
| 18 | Qwen 3.6-27B-AEON-NVFP4 | 57.5/80 |
| 19 | [Muse Glimmer 30B Q4_K_XL (unsloth)](https://huggingface.co/unsloth/Muse-Glimmer-30B-GGUF) | 57/80 |
| 20 | [Muse Glimmer 30B K-Quant Dynamic](https://huggingface.co/meta-models/Muse-Glimmer-30B-GGUF/) | 56/80 |
| 21 | Qwen 3.6-27B-4bit (OpenRouter) | 54/80 |
| 22 | [Ornith-1.0-35B-8bit](https://huggingface.co/deepreinforce-ai/Ornith-1.0-35B-GGUF) | 51/80 |
| 23 | [Ternary Bonsai 27B](https://huggingface.co/prism-ml/Ternary-Bonsai-27B-gguf) | 50.5/80 |
| 24 | Agents A1 FP8 | 48/80 |
| 25 | [Step-3.7-Flash IQ4_XS](https://huggingface.co/stepfun-ai/Step-3.7-Flash-GGUF) | 46/80 |
...
- **Four runners-up at 69/80** with complementary strengths: Flash-Next UD-Q3_K_XL (second-best widgets, 38/40), Qwen 3.8-FP8 (best sync, 36/40), DS-V4-Flash-0731 (37/40 widgets), and Q6K (unique comment high-water mark + strong widgets).
...
- **Claude Opus 4.6** (66.5/80, GitHub Copilot) ties local 8-bit for 6th and beats Sonnet 4.5 by 1.5 points: same focused per-page sync (33/40, no `since` creep) and a slightly stronger widget registry (33.5/40). Held back by silent ambiguity on both tasks and type/instance conflation on widgets.
- **Thinking-on is not uniformly helpful:** on the FP8 checkpoint, `reasoning_effort="medium"` costs 1 point overall (68 vs 69) via incremental-sync scope creep; on the 4bit GGUF it is the difference between 14th (60) and 1st (73), largely by avoiding the thinking-off `since` API bug and by naming both widget pluggability forks.
- **Locally run unsloth 8-bit** (66.5/80) still outperforms all cloud/API variants except Opus, driven by near-perfect widgets (35.5/40).
- **Ambiguity-resolution remains the hardest bar** across all twenty-four models — most implementations pick a path and code it without clearly naming tradeoffs. The 4bit reasoning-medium run is the exception on widgets (10/10).
