...
def load_progress(conn: sqlite3.Connection, repo: str) -> tuple[int, int]:
    """Return (last_page, total_issues) for a repo, or (0, 0) if no checkpoint."""
    row = conn.execute(
        "SELECT last_page, total_issues FROM sync_progress WHERE repo = ?", (repo,)
    ).fetchone()
    if row is None:
        return (0, 0)
    return (row[0], row[1])
...
def save_progress(conn: sqlite3.Connection, repo: str, last_page: int, total_issues: int):
    conn.execute(
        """
        INSERT INTO sync_progress (repo, last_page, total_issues, synced_at)
        VALUES (?, ?, ?, ?)
        ...
        """,
        (repo, last_page, total_issues, datetime.now(timezone.utc).isoformat()),
    )
    conn.commit()
...
def sync_issues(session: requests.Session, conn: sqlite3.Connection, repo: str, per_page: int):
    ...
    start_page, total_issues = load_progress(conn, repo)

    if start_page > 0:
        logger.info("Resuming %s from page %d (previously synced %d issues)", repo, start_page, total_issues)
    else:
