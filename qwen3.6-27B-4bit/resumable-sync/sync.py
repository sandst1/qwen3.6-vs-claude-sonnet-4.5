...
def init_db(conn: sqlite3.Connection):
    conn.executescript(
        ...
        CREATE TABLE IF NOT EXISTS sync_progress (
            ...
            last_issue_id INTEGER DEFAULT 0,
...
def get_progress(conn: sqlite3.Connection, repo: str) -> dict:
    """Get checkpoint for a repo, or defaults if not yet started."""
    row = conn.execute(
        "SELECT last_page, last_issue_id, status FROM sync_progress WHERE repo = ?",
        (repo,),
    ).fetchone()
    if row:
        return {"last_page": row[0], "last_issue_id": row[1], "status": row[2]}
    return {"last_page": 1, "last_issue_id": 0, "status": "in_progress"}


def save_progress(conn: sqlite3.Connection, repo: str, last_page: int, last_issue_id: int, status: str):
    """Checkpoint progress for a repo."""
    conn.execute(
        """
        INSERT INTO sync_progress (repo, last_page, last_issue_id, status)
        VALUES (?, ?, ?, ?)
        ...
        """,
        (repo, last_page, last_issue_id, status),
    )


def clear_progress(conn: sqlite3.Connection):
    """Clear all checkpoints — called on successful full completion."""
...
def sync_issues(session: requests.Session, conn: sqlite3.Connection, repo: str, per_page: int):
    ...
    Resumes from the last checkpoint if the sync was previously interrupted.
    """
    progress = get_progress(conn, repo)

    # Skip repos that completed in a previous run
    if progress["status"] == "completed":
        logger.info("Skipping %s (already synced)", repo)
        return 0

    start_page = progress["last_page"]
    last_issue_id = progress["last_issue_id"]

    if start_page > 1 or last_issue_id > 0:
        logger.info("Resuming %s from page %d (last issue: %s)", repo, start_page, last_issue_id or "none")
    else:
    ...
    while True:
        ...
        for issue in batch:
            # Skip already-synced issues when resuming mid-page
        ...
        if len(batch) < per_page:
            # Last page — mark repo as completed
            save_progress(conn, repo, page, last_issue_id, "completed")
            conn.commit()
...
def main():
    ...
    try:
        ...
        # All repos synced successfully — clear checkpoints so the next
        # run does a fresh sync (to pick up updates).
        clear_progress(conn)
        conn.commit()
        logger.info("Sync complete, checkpoints cleared")
    finally:
        conn.close()
