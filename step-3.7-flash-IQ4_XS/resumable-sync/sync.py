...
def get_sync_state(conn: sqlite3.Connection, repo: str) -> str | None:
    row = conn.execute(
...
def update_sync_state(conn: sqlite3.Connection, repo: str, last_synced_at: str):
    conn.execute(
        """
        INSERT INTO sync_state (repo, last_synced_at)
        VALUES (?, ?)
...
def sync_issues(session: requests.Session, conn: sqlite3.Connection, repo: str, per_page: int):
    ...
    last_synced_at = get_sync_state(conn, repo)
    logger.info("Syncing issues for %s (since %s)", repo, last_synced_at or "beginning")
    url = f"https://api.github.com/repos/{repo}/issues"
    page = 1
    total_issues = 0
    current_synced_at = last_synced_at or "1970-01-01T00:00:00Z"
    ...
    while True:
        ...
        try:
            ...
            logger.error("Failed to fetch page %d for %s: %s", page, repo, e)
            update_sync_state(conn, repo, current_synced_at)
            conn.commit()
            return
        ...
        for issue in batch:
            if upsert_issue(conn, repo, issue):
                ...
                if _normalize_ts(issue_updated_at) > _normalize_ts(current_synced_at):
                    current_synced_at = issue_updated_at
...
def main():
    ...
    try:
        ...
        try:
            ...
            logger.warning("Failed to commit on shutdown", exc_info=True)
        conn.close()
