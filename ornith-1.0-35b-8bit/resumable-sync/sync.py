...
def save_progress(conn: sqlite3.Connection, repo: str, page: int, last_issue_id: int):
    """Persist progress after a page is committed. Called inside the same txn."""
    conn.execute(
        """
        INSERT INTO sync_state (repo, last_page, last_issue_id, completed, updated_at)
        VALUES (?, ?, ?, 0, ?)
        ON CONFLICT(repo) DO UPDATE SET
            ...
            updated_at = excluded.updated_at
        """,
        (repo, page, last_issue_id, datetime.now(timezone.utc).isoformat()),
    )


def load_progress(conn: sqlite3.Connection, repo: str):
    """Return (last_page, last_issue_id, completed) for a repo, or (0, None, False)."""
    row = conn.execute(
        "SELECT last_page, last_issue_id, completed FROM sync_state WHERE repo = ?", (repo,)
    ).fetchone()
    if row is None:
        return 0, None, False
...
def mark_completed(conn: sqlite3.Connection, repo: str):
    row = conn.execute(
        "SELECT last_page, last_issue_id FROM sync_state WHERE repo = ?", (repo,)
    ).fetchone()
    last_page = row[0] if row else 0
    last_issue_id = row[1] if row else None
    conn.execute(
        """
        INSERT INTO sync_state (repo, last_page, last_issue_id, completed, updated_at)
        VALUES (?, ?, ?, 1, ?)
        ON CONFLICT(repo) DO UPDATE SET
            ...
            updated_at = excluded.updated_at
        """,
        (repo, last_page, last_issue_id, datetime.now(timezone.utc).isoformat()),
    )
...
def sync_issues(session: requests.Session, conn: sqlite3.Connection, repo: str, per_page: int):
    ...
    last_page, last_issue_id, completed = load_progress(conn, repo)

    if completed:
        logger.info("Skipping %s (already synced, completed %s)", repo,
                     conn.execute("SELECT updated_at FROM sync_state WHERE repo=?", (repo,)).fetchone()[0])
        return

    if last_page > 0:
        logger.info("Resuming %s from page %d (last issue id: %d)", repo, last_page, last_issue_id)
    ...
    while True:
        ...
        conn.commit()
        save_progress(conn, repo, page, page_last_id)
...
if __name__ == "__main__":
    try:
        ...
        logger.warning("Interrupted by user — progress saved, will resume on next run")
        sys.exit(130)
    except Exception:
