...
Run with:
    python sync.py            # resumes from where it left off
    python sync.py --full     # starts a fresh sync from scratch
"""
import argparse
import logging
import sqlite3
...
def get_resume_page(conn: sqlite3.Connection, repo: str) -> int:
    """Return the page to resume from (1-indexed). Returns 1 if no prior state."""
    row = conn.execute(
        "SELECT last_completed_page FROM sync_state WHERE repo = ?", (repo,)
    ).fetchone()
    if row is None:
        return 1
    # Resume from the page *after* the last fully completed one.
    return row[0] + 1


def save_sync_progress(conn: sqlite3.Connection, repo: str, page: int):
    """Record that we finished processing a page."""
    now = datetime.now(timezone.utc).isoformat()
    conn.execute(
        """
        INSERT INTO sync_state (repo, last_completed_page, started_at)
        VALUES (?, ?, ?)
        ON CONFLICT(repo) DO UPDATE SET last_completed_page = excluded.last_completed_page
        """,
        (repo, page, now),
    )
    # committed together with the issue/comment data by the caller


def clear_sync_state(conn: sqlite3.Connection, repo: str):
    """Remove progress tracking for a repo (call after a full sync completes)."""
...
def sync_issues(session: requests.Session, conn: sqlite3.Connection, repo: str, per_page: int, *, full: bool = False):
    ...
    if full:
        clear_sync_state(conn, repo)

    start_page = get_resume_page(conn, repo)
    if start_page > 1:
        logger.info("Resuming sync for %s from page %d", repo, start_page)
    else:
        logger.info("Starting sync for %s from the beginning", repo)
    ...
    while True:
        ...
        save_sync_progress(conn, repo, page)
        conn.commit()
    ...
    # Full sync completed — clear state so next run starts fresh
    # (picking up any newly created/updated issues).
    clear_sync_state(conn, repo)
    logger.info("Done with %s: %d issues synced this run", repo, total_issues)
...
def main():
    ...
    parser.add_argument("--full", action="store_true", help="Ignore saved progress and start a fresh sync")
