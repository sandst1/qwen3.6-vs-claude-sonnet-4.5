...
def get_checkpoint(conn: sqlite3.Connection, repo: str) -> int | None:
    row = conn.execute(
        "SELECT next_page FROM sync_checkpoints WHERE repo = ?", (repo,)
    ).fetchone()
    return row[0] if row else None


def set_checkpoint(conn: sqlite3.Connection, repo: str, next_page: int):
    conn.execute(
        "INSERT INTO sync_checkpoints (repo, next_page, updated_at) VALUES (?, ?, ?) "
        "ON CONFLICT(repo) DO UPDATE SET next_page = excluded.next_page, updated_at = excluded.updated_at",
        (repo, next_page, datetime.now(timezone.utc).isoformat()),
    )
    conn.commit()
...
def sync_issues(session: requests.Session, conn: sqlite3.Connection, repo: str, per_page: int):
    """Fetch all issues for a repo and write them to the DB, with resumability."""
    logger.info("Syncing issues for %s", repo)
    url = f"https://api.github.com/repos/{repo}/issues"

    checkpoint = get_checkpoint(conn, repo)
    page = checkpoint if checkpoint is not None else 1
    if checkpoint:
    ...
    while True:
        ...
        try:
            ...
            logger.exception("Page %d request failed, will retry next run", page)
            set_checkpoint(conn, repo, page)
            return
        ...
        for issue in batch:
            try:
                ...
                logger.exception("Skipping bad issue #%d", issue.get("number", "?"))
        ...
        set_checkpoint(conn, repo, page + 1)
        page += 1
...
def fetch_comments_for_issue(session: requests.Session, conn: sqlite3.Connection, issue: dict):
    ...
    while True:
        try:
            ...
            logger.exception("Failed to fetch comments page %d for issue #%d, skipping remaining", page, issue.get("number", "?"))
            conn.commit()
            return
